...
export default function AnalyticsTasks({ rem , title, subheader,set_rem ,  list, ...other }) {
  const [selected, setSelected] = useState([]);
  const now_context = useContext(curr_context) ;  
  useEffect(() => {
    const initialSelected = list.filter(task => task.done).map(task => task.id);
  ...
  const handleClickComplete = (taskId) => {
    ...
    const run = async () => {
      const url = `${now_context.backend_url}/trainee/events/${taskId}/${selected.includes(taskId)}`;
      try {
        ...
        console.log(response); // Log the response data or handle it as needed
      } catch (error) {
    ...
    run();
    if(selected.includes(taskId)) set_rem(rem+1) ; 
    else set_rem(rem-1)
